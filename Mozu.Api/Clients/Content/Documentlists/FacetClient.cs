...
namespace Mozu.Api.Clients.Content.Documentlists
{
	...
	public partial class FacetClient 	{
		...
		/// <example>
		/// <code>
		///   var mozuClient=GetFacets(dataViewMode,  documentListName,  propertyName, authTicket);
		///   var facetClient = mozuClient.WithBaseAddress(url).Execute().Result();
		/// </code>
		/// </example>
		public static MozuClient<List<Mozu.Api.Contracts.Content.Facet>> GetFacetsClient(DataViewMode dataViewMode, string documentListName, string propertyName, AuthTicket authTicket= null)
		{
			var url = Mozu.Api.Urls.Content.Documentlists.FacetUrl.GetFacetsUrl(documentListName, propertyName);
			const string verb = "GET";
			var mozuClient = new MozuClient<List<Mozu.Api.Contracts.Content.Facet>>().WithVerb(verb).WithResourceUrl(url).WithHeader(Headers.X_VOL_DATAVIEW_MODE ,dataViewMode.ToString());
			if (authTicket != null)
